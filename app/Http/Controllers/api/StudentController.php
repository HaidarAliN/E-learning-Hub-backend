...
class studentController extends Controller
{
    public function getCourseByName(Request $request){
        ...
        $enroled_courses = User::find($user_id)//return a list of all the id's of the courses that the student is enrolled or pending to join
                                ->enrolledCourses()
                                ->pluck('id');
        $courses = Course::where('courses.name', 'like', "$name")
                            ->whereNotIn('courses.id',$enroled_courses)//exclude all the courses from the above list
    ...
    public function getOngoingCourses(){
        $user_id = auth()->user()->id;
        $enroled_courses =  DB::Table('participants')
                                ->select('course_id as id')
                                ->where([['status',1],['user_id',$user_id]])
                                ->pluck('id');//get the list of id's for all enrolled courses
    ...
    public function enrollInCourse(Request $request){
        ...
        User::find($user_id)
                ->enrolledCourses()
                ->save($course);//enroll student in course
    ...
    public function sendNotification($tokento, $title, $subject)
    {
        ...
        $msg = array
              (
                ...
                'title' => "$title",
                'receiver' => 'erw',
                'icon'  => "https://www.pngitem.com/pimgs/m/287-2874123_ios-icon-student-registration-icon-hd-png-download.png",/*Default Icon*/
